...
| [dashboard-implementation-plan.md](dashboard-implementation-plan.md) | 2026-03 | The plan for the aiohttp dashboard, listing the endpoints and files it would add. The endpoints and the UI were implemented; the proposed `DashboardConfig` dataclass was not — `core/config.py` has only a flat `dashboard_port` field, with no `host` or `enabled` setting. |
